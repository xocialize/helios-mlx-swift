...
| **S3b** AR loop | 33-frame-chunk generation (`generate_helios.py`): spatial helpers (reshape, bilinear-down/nearest-up, area downsample), rolling 19f history buffer + `_split_history` + prefix, 3-stage 1/4-res pyramid loop w/ DMD steps + alpha/beta block-noise mix. **Parity wrinkle:** `sample_block_noise` uses NUMPY randomness (Cholesky-correlated) — NOT mx.random — so e2e parity must INJECT block noise (and the seeded initial noise) from fixtures. Gate: per-chunk denoise (injected noise) vs oracle → full e2e → VAE decode (reuse wan-core 16-ch WanVAE) → GPU smoke. |
